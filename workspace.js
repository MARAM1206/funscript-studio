...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SNAP_DIST = 12; 
    const GAP = 10;       
    const VERSION = 'funscript_workspace_layout_v16'; 
    ...
    function saveLayout() {
        panels.forEach(panel => {
            layoutState[panel.id] = { left: panel.offsetLeft, top: panel.offsetTop, width: panel.offsetWidth, height: panel.offsetHeight, visible: panel.style.display !== 'none' };
        });
    ...
    if (restoreLayoutBtn) {
        restoreLayoutBtn.addEventListener('click', (e) => {
            ...
            if (custom) localStorage.setItem(VERSION, JSON.stringify(custom));
            else { alert("Restaurando al valor por defecto."); localStorage.setItem(VERSION, JSON.stringify(getDefaultLayout())); }
            location.reload(); 
        });
    }
    ...
    if (fullscreenLayoutBtn) {
        fullscreenLayoutBtn.addEventListener('click', (e) => {
            e.preventDefault(); localStorage.setItem(VERSION, JSON.stringify(getDefaultLayout())); location.reload(); 
        });
    }
    ...
    if (cacheBtn) {
        cacheBtn.addEventListener('click', (e) => {
            e.preventDefault();
            if (confirm('¿Borrar caché temporal? (Tus Presets y Acomodo NO se perderán)')) {
                for (let i = localStorage.length - 1; i >= 0; i--) {
                    const key = localStorage.key(i);
                    if (key !== 'funscript_presets' && key !== 'funscript_layout_user_custom') localStorage.removeItem(key);
                }
    ...
    panels.forEach(panel => {
        const id = panel.id; const title = panel.getAttribute('data-title');
        if (permanentPanels.includes(id) && layoutState[id]) layoutState[id].visible = true;
        const state = layoutState[id] || getDefaultLayout()[id];
        
        if (state.visible) panel.style.display = 'flex'; else panel.style.display = 'none';

        if (!permanentPanels.includes(id)) {
            const btn = document.createElement('button');
            btn.className = 'toggle-panel-btn'; btn.innerText = title;
            if (state.visible) btn.classList.add('active');
            togglesContainer.appendChild(btn);

            btn.addEventListener('click', () => {
                const isVisible = panel.style.display !== 'none';
                if (isVisible) { panel.style.display = 'none'; btn.classList.remove('active'); } 
                else { panel.style.display = 'flex'; btn.classList.add('active'); panel.style.zIndex = ++highestZIndex; }
                saveLayout();
            });
        }

        const directions = ['n', 's', 'e', 'w', 'ne', 'nw', 'se', 'sw'];
        directions.forEach(dir => { const handle = document.createElement('div'); handle.className = `resize-handle resize-handle-${dir}`; panel.appendChild(handle); });
        ...
        panel.style.left = safeL + 'px'; panel.style.top = safeT + 'px';
        ...
        if (header) {
            header.addEventListener('mousedown', (e) => {
                ...
                e.preventDefault(); panel.style.zIndex = ++highestZIndex;
                let startX = e.clientX; let startY = e.clientY; let startLeft = panel.offsetLeft; let startTop = panel.offsetTop;
                let container = document.documentElement;
                document.body.style.userSelect = 'none';

                const onMouseMove = (ev) => {
                    let newL = startLeft + (ev.clientX - startX); let newT = startTop + (ev.clientY - startY);
                    const WW = window.innerWidth; const HH = window.innerHeight - topBarH;
                    newL = Math.max(GAP, Math.min(newL, WW - panel.offsetWidth - GAP));
                    newT = Math.max(GAP, Math.min(newT, HH - panel.offsetHeight - GAP));

                    let newR = newL + panel.offsetWidth; let newB = newT + panel.offsetHeight;
                ...
                const onMouseUp = () => { document.removeEventListener('mousemove', onMouseMove); document.removeEventListener('mouseup', onMouseUp); document.body.style.userSelect = ''; saveLayout(); };
        ...
        handles.forEach(handle => {
            handle.addEventListener('mousedown', (e) => {
                e.preventDefault(); e.stopPropagation(); panel.style.zIndex = ++highestZIndex; document.body.style.userSelect = 'none';
                const type = handle.className.split(' ').find(c => c.startsWith('resize-handle-')).replace('resize-handle-', '');
                const startX = e.clientX; const startY = e.clientY;
                const startW = panel.offsetWidth; const startH = panel.offsetHeight;
                const startL = panel.offsetLeft; const startT = panel.offsetTop;
                ...
                const onMouseMove = (ev) => {
                    ...
                    if (type.includes('w')) {
                        ...
                        let pW = startW + (startL - snappedL);
                        if (pW >= minW) { nw = pW; nl = snappedL; } else { nw = minW; nl = startL + (startW - minW); }
                    }
                    if (type.includes('n')) {
                        ...
                        let pH = startH + (startT - snappedT);
                        if (pH >= minH) { nh = pH; nt = snappedT; } else { nh = minH; nt = startT + (startH - minH); }
                    }

                    panel.style.width = nw + 'px'; panel.style.left = nl + 'px';
                    panel.style.height = nh + 'px'; panel.style.top = nt + 'px';
