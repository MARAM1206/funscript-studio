...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function applyTheme(isLight) {
        ...
        themeMenuBtn.innerHTML = isLight ? '🌙 Modo oscuro' : '☀️ Modo claro';
    ...
    const GAP = 8; const SNAP_DIST = 15; let topZIndex = 100;
    ...
    if (!savedLayout || Object.keys(savedLayout).length !== 6) {
        ...
        for (let id in defaultLayout) {
            savedLayout[id] = { left: `${defaultLayout[id].left}px`, top: `${defaultLayout[id].top}px`, width: `${defaultLayout[id].width}px`, height: `${defaultLayout[id].height}px` };
        }
    ...
    panels.forEach(panel => {
        ...
        if (pos) { panel.style.left = pos.left; panel.style.top = pos.top; panel.style.width = pos.width; panel.style.height = pos.height; }
        panel.addEventListener("mousedown", () => { topZIndex++; panel.style.zIndex = topZIndex; });
        ...
        if (!panel.querySelector(".panel-header")) {
            ...
            header.addEventListener("mousedown", (e) => {
                ...
                startX = e.clientX; startY = e.clientY; startLeft = panel.offsetLeft; startTop = panel.offsetTop;

                const onMouseMove = (moveEvent) => {
                    if (!isDragging) return;
                    let newLeft = startLeft + (moveEvent.clientX - startX); let newTop = startTop + (moveEvent.clientY - startY);
    ...
    function saveCurrentLayout() {
        const layoutToSave = {};
        panels.forEach(panel => {
            layoutToSave[panel.id] = { left: panel.style.left, top: panel.style.top, width: panel.style.width, height: panel.style.height };
        });
